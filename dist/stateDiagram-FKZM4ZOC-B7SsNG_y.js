import { s as G, a as W, S as N } from "./chunk-DI55MBZ5-YBfWixkp.js";
import { _ as f, c as t, d as H, l as S, e as P, k as z, R as _, u as U } from "./mermaid.core-BXO_VlFI.js";
import { G as C } from "./graph-AjkVv7YK.js";
import { l as F } from "./layout-BOEOBhW3.js";
import { l as O, k as J } from "./string-BnSFgVSn.js";
var X = /* @__PURE__ */ f((e) => e.append("circle").attr("class", "start-state").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit).attr("cy", t().state.padding + t().state.sizeUnit), "drawStartState"), D = /* @__PURE__ */ f((e) => e.append("line").style("stroke", "grey").style("stroke-dasharray", "3").attr("x1", t().state.textHeight).attr("class", "divider").attr("x2", t().state.textHeight * 2).attr("y1", 0).attr("y2", 0), "drawDivider"), Y = /* @__PURE__ */ f((e, i) => {
  ...
  const n = i.points, l = O().x(function(s) {
    ...
    return s.y;
  }).curve(J), x = e.append("path").attr("d", l(n)).attr("id", "edge" + R).attr("class", "transition");
  let a = "";
  if (t().state.arrowMarkerAbsolute && (a = _(!0)), x.attr(
    ...
    if (o.length > 1) {
      ...
      B.forEach((h, y) => h.attr("y", p + y * g - u)), E = g * o.length;
    }
    const r = s.node().getBBox();
    s.insert("rect", ":first-child").attr("class", "box").attr("x", w - m / 2 - t().state.padding / 2).attr("y", p - E / 2 - t().state.padding / 2 - 3.5).attr("width", m + t().state.padding).attr("height", E + t().state.padding), S.info(r);
  }
  R++;
  ...
  for (const r of g) {
    ...
    if (u.doc) {
      ...
      h = A(u.doc, y, u.id, !c, n, l, x);
      {
        ...
        h.width = k.width, h.height = k.height + b.padding / 2, T[u.id] = { y: b.compositTitleSize };
      }
    } else
      h = L(i, u, a);
    if (u.note) {
